...
class LiveEngine:
    ...
    def _ws_on_exit(self, symbol: str, exit_price: float, reason: str,
                    hold_seconds: float, entry_price: float,
                    qty: float, entry_time: str):
        """WebSocket触发出场时的回调，在独立线程中执行"""
        mode = self.cfg.get("mode", "paper")
        ...
        self._log(symbol,
            f"WS出场:{reason} 持仓{hold_seconds}s "
            f"均价={entry_price:.6f} 卖出={exit_price:.6f} "
            f"PnL={pnl_pct:+.2f}% ({pnl_usdt:+.4f}U)")

    def _get_current_price(self, symbol: str) -> float:
        """REST轮询获取当前价格（paper模式或WS降级时使用）"""
        try:
